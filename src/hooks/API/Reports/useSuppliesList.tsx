...
export const useSuppliesReportsList = (active: boolean) => {
  ...
  const {
    result,
    loading: isLoading,
    execute: getSuppliesReports,
  } = useGetSuppliesReports({
    onError: () => showError('Ошибка при получении отчета'),
  })

  const fetchSuppliesReports = useCallback(
    (resetPage = false) => {
    ...
    [page, limit, dateRange, getSuppliesReports, setQuery],
  )

  useEffect(() => {
    if (!active) return
    fetchSuppliesReports()
  }, [page, limit, dateRange, active, fetchSuppliesReports])
