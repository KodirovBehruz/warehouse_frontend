...
export const useWriteOffReportsList = (active: boolean) => {
  ...
  const {
    result,
    loading: isLoading,
    execute: getWriteOffReports,
  } = useGetWriteOffReports({
    onError: () => showError('Ошибка при получении отчета'),
  })

  const fetchWriteOffReports = useCallback(
    (resetPage = false) => {
    ...
    [page, limit, dateRange, getWriteOffReports, setQuery],
  )

  useEffect(() => {
    if (!active) return
    fetchWriteOffReports()
  }, [page, limit, dateRange, active, fetchWriteOffReports])
