...
import { IProductCreateContract } from "../../../models/delivery/contracts/IProducts.ts";
...
export const CreateWriteOffModal: FC<ICreateWriteOffModal> = ({ isOpen, setIsOpen, onCreateSuccess }) => {
    const delivery = useDelivery();
    const [form] = Form.useForm<IWriteOffCreateContract>();
    const { error: showError, success: showSuccess } = useMessage();


    const { result, loading: isProductsLoading, execute: getProducts } = useFetch({
    ...
    return (
        ...
        >
            <Form
                ...
                layout='vertical'
                onFinish={submitForm}>
                <Form.Item label='Выберите товар' name='productId' rules={[{ required: true, message: "Выберите товар" }]}>
                    <Select
                ...
                <Form.Item label='Причина списания' name='reason' rules={[{ required: true, message: "Введите причину" }]}>
                    <Input />
                </Form.Item>
            </Form>
        </Modal>
