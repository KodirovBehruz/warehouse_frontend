...
export const CreateSupplyModal: FC<ICreateSupplyModal> = ({ isOpen, setIsOpen, onCreateSuccess }) => {
        ...
        >
            <Form
                ...
                {selectedProducts.map(({ id, quantity }) => (
                    <Form.Item key={id} label={`Количество для ${products.find(p => p.id === id)?.name || ''}`}>
                        <Input
                            type='number'
                            value={quantity}
